...
class ShPaymentsScreenState extends State<ShPaymentsScreen> {
  ...
  isCardProviderEmpty() {
    return (providerCard.cardNo == '' ||
        providerCard.cvv == '' ||
        providerCard.month == '' ||
        providerCard.year == '' ||
        providerCard.holderName == '');
  }

  init() async {
    providerCard =
        Provider.of<OrdersProvider>(context, listen: false).getCard();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: isInProgress
          ? Center(
              child: CircularProgressIndicator()) //loading widget goes here
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        InkWell(
                          onTap: () async {
                            if (Provider.of<OrdersProvider>(context,
                                        listen: false)
                                    .getCard()
                                    .cardNo !=
                                '') {
    ...
    if (!isCardProviderEmpty()) {
      return Container(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            text('Holder Name : ' + holderName,
                fontSize: textSizeMedium, textColor: sh_textColorPrimary),
            text('Card Number : ' + cardNo,
                fontSize: textSizeMedium, textColor: sh_textColorPrimary),
            text('CVV : ' + cvv,
                fontSize: textSizeMedium, textColor: sh_textColorPrimary),
            text('Exp Date : ' + year + ' / ' + month,
                fontSize: textSizeMedium, textColor: sh_textColorPrimary),
          ],
        ),
      );
  ...
  sendOrderInfo() async {
    ...
    ShippingMethod shipping_method =
        Provider.of<OrdersProvider>(context, listen: false).getShippingMethod();
    double totalAmount = Provider.of<OrdersProvider>(context, listen: false)
        .getTotalPriceSimple();
    ...
    MyResponse myResponse = await PaymentController.sendOrderInfo(
        items: items,
  ...
  Future<bool> registerAndLogin() async {
    MyResponse myResponse = await AuthController.registerUser(
        address.name, address.email, 'password');
    if (myResponse.success) {
      // toasty(context, 'You registered with ${address.email}',duration: Duration(seconds: 3));
      // print('You registered with ${address.email}');
      MyResponse response =
          await AuthController.loginUser(address.email, 'password');
      if (response.success) {
